...
def _save_profile_to_db(state: OBState) -> None:
    ...
    try:
        profile_data = state.get("profile", {})
        payload = {k: v for k, v in profile_data.items() if k in {
            "user_id", "user_role", "industry", "company_size", "website_status", 
        ...
        if user_id:
            ...
            print(f"[onboarding] saved profile for {user_id}")
    except Exception as e:
...
def _current_ui(user_id: str) -> UIBlock:
    ...
    try:
        ...
        # Check for UI in state values first
        ui = vals.get("ui")
        if ui:
            return ui
        ...
        print(f"[onboarding] Error getting UI state: {e}")
        return {
            "ui_type": "input", 
            "message": "حياك الله! أنا MORVO 🤝 مستشارتك الذكية للتسويق في السوق السعودي 🇸🇦\n\nخلّينا نبدأ بالتعارف… وش اسمك الأول؟"
        }
...
def start_onboarding(user_id: str) -> Dict[str, Any]:
    ...
    print(f"[onboarding] starting onboarding for user_id: {user_id}")
    ...
    try:
        ...
        print(f"[onboarding] Error starting onboarding: {e}")
        return {
            "conversation_id": f"onb:{user_id}",
            "done": False,
            "ui": {
...
def resume_onboarding(user_id: str, value: Any) -> Dict[str, Any]:
    ...
    print(f"[onboarding] resuming onboarding for user_id: {user_id} with value: {value}")
    ...
    try:
        ...
        if snap.next is None:
            print(f"[onboarding] onboarding completed for user_id: {user_id}")
            return {
                "conversation_id": f"onb:{user_id}",
                "done": True,
                "ui": None
        ...
        print(f"[onboarding] Error resuming onboarding: {e}")
        return {
            "conversation_id": f"onb:{user_id}",
            "done": False,
            "ui": {
